...
//
// FILE: 1_SurfaceWater_v5.2b_unified.js
// VERSION: v5.2b_unified
//
// PURPOSE
...
Then choose the raster band set:
  ...
  var EXPORT_PROFILE = 'BASIC';     // binary water mask only
  var EXPORT_PROFILE = 'CORE';      // smallest operational product with traceability
...
EXPORT_PROFILE = 'BASIC'
  Exports only the binary monthly water band:
    water
  This is the lightest raster option. It keeps the same processing logic, but
  does not export validity, source, gap or diagnostic bands.
...
Change to BASIC when you only need the final binary water mask.
...
function profileIsValid() {
  return EXPORT_PROFILE === 'BASIC' || EXPORT_PROFILE === 'CORE' || EXPORT_PROFILE === 'STANDARD' || EXPORT_PROFILE === 'FULL';
}
...
if (!profileIsValid()) {
  print('WARNING: EXPORT_PROFILE must be BASIC, CORE, STANDARD or FULL. Current value:', EXPORT_PROFILE);
}
if (!runModeIsValid()) {
...
function buildExportImage(core) {
  ...
  // BASIC profile: export only the final binary water band.
  // The water band keeps the current explicit semantics:
  //   1 = water; 0 = valid non-water; masked = no valid final estimate.
  if (EXPORT_PROFILE === 'BASIC') {
    return exportImage;
  }
...
print('Running unified script:', '1_SurfaceWater_v5.2b_unified.js');
print('VERSION:', 'v5.2b_unified');
