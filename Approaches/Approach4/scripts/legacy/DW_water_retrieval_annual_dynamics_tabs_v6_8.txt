/**** Annual water-dynamics metric-specific lightweight application v6.8 ****/
/**** Standalone GEE app or annual-dynamics tab appended to DW_water_retrieval.js. ****/
...
Purpose
-------
Calculate annual timing, duration, recurrence, support and provenance metrics
for user-selected surface-water classes using temporally ordered observations
...
function dynSetBusy(isBusy, message) {
  if (typeof runDynamicsButton !== 'undefined') {
    ...
    runDynamicsButton.setLabel(
      isBusy ? 'Calculating annual dynamics…' : 'Run selected annual dynamics'
    );
  }
...
function dynStartDrawing(shape) {
  ...
  dynSetStatus(
    'Draw the ' + shape + ' on the map, then run the annual analysis.'
  );
}
...
function dynRenderCoverageDiagnostics() {
  ...
  dynInspectorChartPanel.add(ui.Label({
    value: 'Reading annual coverage and exclusion counts…',
  ...
  summary.evaluate(function(result, error) {
    ...
    dynInspectorChartPanel.add(ui.Label({
      value: source + ' annual dates at this pixel — asset footprint: ' +
    ...
    buttonPanel.add(ui.Button({
      ...
      onClick: function() {
        ...
        Export.table.toDrive({
          ...
          description: 'HLS_platform_verification_' + lastDynamicsYear,
          folder: dynamicsDriveFolder.getValue(),
          fileNamePrefix: 'HLS_platform_verification_' + lastDynamicsYear,
          fileFormat: 'CSV'
        });
        dynSetStatus(
...
function dynRenderInspectorChart() {
  ...
  if (
    ...
    dynInspectorChartPanel.add(ui.Label(
      'Run the annual analysis, enable the inspector and click a pixel.'
    ));
    return;
  }
...
function dynInspectCoordinates(coords) {
  ...
  if (!dynEnsureLazyInspectorData(dynInspectorPoint)) {
    dynSetStatus(
      'Run the annual dynamics before using the pixel time-series inspector.'
    );
    return;
  }
  ...
  dynInspectorInfoLabel.setValue(
    'Year ' + lastDynamicsYear + ' | lon ' + coords.lon.toFixed(5) +
...
function dynUpdateInspectorHelp() {
  ...
  if (mode === DYN_INSPECTOR_EVOLUTION_MODE) {
    message = 'The stepped profile shows the sequence of observed classes: ' +
      ...
      'was not selected as an annual target. The connecting steps are a ' +
...
function dynDoyFromDate(year, monthIndex, day) {
  var start = Date.UTC(year, 0, 0);
  var current = Date.UTC(year, monthIndex, day);
  return Math.floor((current - start) / 86400000);
}

function dynContinuousLegendSpec(type, year) {
  if (type === 'DOY') {
    return {
      palette: DYN_DOY_VIS.palette,
      ticks: [
        '1 Jan\nDOY 1',
        '1 Apr\nDOY ' + dynDoyFromDate(year, 3, 1),
        '1 Jul\nDOY ' + dynDoyFromDate(year, 6, 1),
        '1 Oct\nDOY ' + dynDoyFromDate(year, 9, 1),
        '31 Dec\nDOY ' + dynDoyFromDate(year, 11, 31)
      ],
      description: 'Day of year: purple indicates earlier dates and yellow later dates.',
      note: 'DOY is the ordinal calendar day. The displayed dates follow the selected analysis year.'
    };
  }
  ...
  if (type === 'DURATION') {
    return {
      ...
      note: 'The legend is a fixed annual display stretch from 0 to 366 days.'
    };
  }
...
function dynRenderLegend(metricLabel) {
  ...
  var spec = dynContinuousLegendSpec(entry.type, entry.year);
...
dynamicsPanel.add(ui.Label({
  value: 'Annual water and inundation dynamics',
  style: {
    fontWeight: 'bold',
    margin: '8px 0 4px 0'
  }
...
dynamicsPanel.add(ui.Label(
  ...
  'exports from separate tabs. The processing methods are unchanged.'
));
...
dynamicsAnalysisPanel.add(ui.Label(
  'Choose the AOI, classes, sources, year and output metrics, then run the analysis.'
));
...
// ------------------------------------------------------
// Analysis year and episode definition
// ------------------------------------------------------

dynamicsAnalysisPanel.add(ui.Label({
  value: '3. Year and episode settings',
  style: {
    fontWeight: 'bold',
    margin: '10px 0 3px 0'
  }
...
var dynamicsYearBox = ui.Textbox({
  placeholder: 'YYYY',
  value: '2025',
  style: {width: '80px'}
});
...
var dynFirstWet = ui.Checkbox({
  label: 'First observed selected-water day (DOY)',
  value: true
});

var dynLastWet = ui.Checkbox({
  label: 'Last selected water-class day (DOY)',
  value: false
});

var dynFirstDryAfterWet = ui.Checkbox({
  label: 'First dry day after final selected detection (DOY)',
  value: false
});

var dynFirstOpen = ui.Checkbox({
  label: 'First observed open-water day (DOY)',
  value: false
});

var dynLastOpen = ui.Checkbox({
  label: 'Last observed open-water day (DOY)',
  value: false
});

var dynFirstPartial = ui.Checkbox({
  label: 'First observed inundated/partial day (DOY)',
  value: false
});

var dynLastPartial = ui.Checkbox({
  label: 'Last observed inundated/partial day (DOY)',
  value: false
});
    ...
    if (enabled) {
      dynSetStatus(
        (lastDynamicsObservations === null && lastDynamicsLazyRunConfig === null) ?
          'Pixel inspector enabled. Run the annual analysis, then click a pixel.' :
...
var runDynamicsButton = ui.Button({
  label: 'Run selected annual dynamics',
  style: {
    stretch: 'horizontal',
    margin: '10px 0 4px 0'
  },
...
var dynamicsDriveFolder = ui.Textbox({
  ...
  value: 'GEE_annual_water_dynamics',
  style: {stretch: 'horizontal'}
});

var dynamicsAssetCollection = ui.Textbox({
  placeholder: 'users/your_user/annual_water_dynamics',
  value: 'users/your_user/annual_water_dynamics',
  style: {stretch: 'horizontal'}
});
...
if (
  ...
  tabSelect.items().add('Annual water dynamics');
  ...
  tabSelect.onChange(function(tabName) {
    ...
    if (tabName === 'Annual water dynamics') {
...
// the inspector checkbox is enabled and an annual run has been completed.
Map.onClick(function(coords) {
  dynInspectCoordinates(coords);
});

// ------------------------------------------------------
// Annual-dynamics collection construction
// ------------------------------------------------------
...
This path intentionally carries only one DOY band. Class-0 dry validity,
...
function dynDirectMaskedDoy(name) {
...
function dynDirectImageDoy(image, condition, outputName) {
  var date = ee.Date(image.get('system:time_start'));
...
function dynPrepareDirectSources(aoi, year) {
...
function dynDirectExtremeForSource(
  ...
  if (!enabled) {
    return dynDirectMaskedDoy(outputName);
  }

  var candidates = ee.ImageCollection(source.map(function(image) {
    return dynDirectImageDoy(
      image,
  ...
  var placeholder = dynDirectMaskedDoy(outputName)
...
function dynBuildDirectTimingImages(
  ...
  return {
    firstSelected: build(
      'selected', 'min', 'first_selected_class_doy', needs.firstSelected
    ),
    lastSelected: build(
      'selected', 'max', 'last_selected_class_doy', needs.lastSelected
    ),
    firstOpen: build(
      'open', 'min', 'first_open_water_doy', needs.firstOpen
    ),
    lastOpen: build(
      'open', 'max', 'last_open_water_doy', needs.lastOpen
    ),
    firstPartial: build(
      'partial', 'min', 'first_inundated_partial_doy', needs.firstPartial
    ),
    lastPartial: build(
      'partial', 'max', 'last_inundated_partial_doy', needs.lastPartial
    )
  };
}
...
function dynBuildDirectFirstDryAfterLastSelected(
  ...
  var candidates = ee.ImageCollection(joinedDates.map(function(dayElement) {
    ...
    var doy = ee.Image.constant(
      ...
      .rename('first_dry_candidate_doy')
      .toInt16();

    // doy.gt(lastSelected) also propagates the mask of lastSelected. Pixels
    // without any selected-water detection therefore remain masked.
    return doy
      .updateMask(fusedDry.eq(1))
      .updateMask(doy.gt(lastSelected))
      .set({
        date_key: representative.get('date_key'),
        'system:time_start': representative.get('system:time_start')
      });
  ...
  return candidates
    ...
    .rename('first_dry_after_final_selected_doy')
    .toInt16();
}
...
Under "Any selected water wins", this engine carries only:
  ...
  doy      = UTC acquisition date expressed as day of year.
...
function dynBuildDirectSelectedValidCollection(
  ...
  return ee.ImageCollection(joinedDates.map(function(dayElement) {
    ...
    return selectedAny
      ...
      .addBands(doy)
      .updateMask(validAny.eq(1))
      .select(['selected', 'valid', 'doy'])
      .cast(
        {selected: 'uint8', valid: 'uint8', doy: 'int16'},
        ['selected', 'valid', 'doy']
      )
      .set({
        date_key: representative.get('date_key'),
...
function buildAnnualDynamicsCollection(aoi, year, sourceOptions, classOptions) {
  var start = ee.Date.fromYMD(year, 1, 1);
  var end = start.advance(1, 'year');
  ...
  var metricCollection = ee.ImageCollection(joined.map(function(dayElement) {
    ...
    var date = ee.Date(representative.get('system:time_start'));
    var doy = observation.select('valid')
      .multiply(0)
      .add(date.getRelative('day', 'year').add(1))
      .rename('doy')
      .toInt16()
...
// ------------------------------------------------------
// Annual-dynamics metric helpers
// ------------------------------------------------------
...
function dynConditionDoySingle(
  collection,
  conditionBand,
  reducerName,
  outputName
...
function dynConditionDoyMinMax(
  collection,
  conditionBand,
  firstOutputName,
  lastOutputName
) {
  // Build one conditional DOY collection and obtain the minimum and maximum
  ...
  return {
    first: minMax.select('condition_doy_min')
      .rename(firstOutputName)
      .toInt16(),
    last: minMax.select('condition_doy_max')
...
function dynFirstDryAfterLastWet(collection, lastWet) {
  return collection.map(function(image) {
    ...
    return doy.updateMask(candidate);
  })
  .reduce(ee.Reducer.min(), DYN_REDUCE_PARALLEL_SCALE)
  .rename('first_dry_after_final_selected_doy')
  .toInt16();
...
function dynSourceAtDoy(collection, targetDoy, outputName) {
  return collection.map(function(image) {
    var onTargetDate = image.select('doy').eq(targetDoy);
...
function dynSequenceMetrics(collection, maxGapDays) {
  ...
  var result = ee.Image(collection.iterate(function(item, accumulator) {
    ...
    var doy = image.select('doy').unmask(0).toInt16();

    var previousDoy = acc.select('previous_doy');
    var previousSelected = acc.select('previous_selected');
    var hasPrevious = acc.select('has_previous');
    var previousRunStart = acc.select('run_start_doy');

    var gap = doy.subtract(previousDoy);
    ...
    runStart = runStart.where(startsEpisode, doy);

    var currentRunDays = doy
      .subtract(runStart)
      .add(1)
    ...
    var nextPreviousDoy = previousDoy.where(valid, doy);
...
// ------------------------------------------------------
// Run and export annual dynamics
// ------------------------------------------------------
...
function runAnnualDynamics() {
  ...
  var year = Number(dynamicsYearBox.getValue());

  if (!year || year < 2015 || year > 2100) {
    dynSetStatus('Enter a valid year, 2015 or later.');
    return;
  }
  ...
  var directSources = dynPrepareDirectSources(aoi, year);
  ...
  var firstSelected = needFirstSelected ? (
    useDirectTiming ? directTiming.firstSelected : dynConditionDoySingle(
      observations, 'selected', 'min', 'first_selected_class_doy'
    )
  ) : null;
  var lastSelected = needLastSelected ? (
    useDirectTiming ? directTiming.lastSelected : dynConditionDoySingle(
      observations, 'selected', 'max', 'last_selected_class_doy'
    )
  ) : null;
  var firstOpen = needFirstOpen ? (
    useDirectTiming ? directTiming.firstOpen : dynConditionDoySingle(
      observations, 'open', 'min', 'first_open_water_doy'
    )
  ) : null;
  var lastOpen = needLastOpen ? (
    useDirectTiming ? directTiming.lastOpen : dynConditionDoySingle(
      observations, 'open', 'max', 'last_open_water_doy'
    )
  ) : null;
  var firstPartial = needFirstPartial ? (
    useDirectTiming ? directTiming.firstPartial : dynConditionDoySingle(
      observations, 'partial', 'min', 'first_inundated_partial_doy'
    )
  ) : null;
  var lastPartial = needLastPartial ? (
    useDirectTiming ? directTiming.lastPartial : dynConditionDoySingle(
      observations, 'partial', 'max', 'last_inundated_partial_doy'
    )
  ) : null;
  ...
  if (dynFirstWet.getValue()) {
    addSelected(
      firstSelected,
      DYN_DOY_VIS,
      'First observed selected-water day (DOY)',
      'DOY'
    );
  }
  ...
  if (dynLastWet.getValue()) {
    addSelected(
      lastSelected,
      DYN_DOY_VIS,
      'Last selected water-class day (DOY)',
      'DOY'
    );
  }
  ...
  if (dynFirstDryAfterWet.getValue()) {
    addSelected(
      ...
      DYN_DOY_VIS,
      'First dry day after final selected detection (DOY)',
      'DOY'
    );
  }
  ...
  if (dynFirstOpen.getValue()) {
    addSelected(
      firstOpen,
      DYN_DOY_VIS,
      'First observed open-water day (DOY)',
      'DOY'
    );
  }
  ...
  if (dynLastOpen.getValue()) {
    addSelected(
      lastOpen,
      DYN_DOY_VIS,
      'Last observed open-water day (DOY)',
      'DOY'
    );
  }
  ...
  if (dynFirstPartial.getValue()) {
    addSelected(
      firstPartial,
      DYN_DOY_VIS,
      'First observed inundated/partial day (DOY)',
      'DOY'
    );
  }
  ...
  if (dynLastPartial.getValue()) {
    addSelected(
      lastPartial,
      DYN_DOY_VIS,
      'Last observed inundated/partial day (DOY)',
      'DOY'
    );
  }
  ...
  lastDynamicsName = 'annual_selected_water_dynamics_' + year;
  lastDynamicsBands = bandNames;
  lastDynamicsAoi = aoi;
  lastDynamicsYear = year;
  ...
  dynSetBusy(
    false,
    'Stage 3/3: metric-specific preview queued for ' + year + '. The ' +
    (useDirectTiming ?
...
function exportAnnualDynamics() {
  if (lastDynamicsImage === null) {
    dynSetStatus(
      'No annual dynamics image is available. Run the annual dynamics first.'
    );
    return;
  }
