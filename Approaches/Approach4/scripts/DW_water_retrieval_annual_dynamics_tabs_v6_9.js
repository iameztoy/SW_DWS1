/**** Hydrological-period water-dynamics metric-specific lightweight application v6.9 ****/
...
function dynConfigureFirstSelectedTimingPreview(entry, mode) {
...
function dynRefreshFirstSelectedTimingPreview() {
  if (typeof dynamicsFirstSelectedTimingVis === 'undefined' ||
      dynamicsFirstSelectedTimingVis === null) {
    return;
  }

  var mode = dynamicsFirstSelectedTimingVis.getValue();
  var targetLabel = null;

  dynPreviewProductOrder.some(function(label) {
    var candidate = dynPreviewProducts[label];
    if (candidate && candidate.isFirstSelectedTiming) {
      targetLabel = label;
      return true;
    }
    return false;
  ...
  var entry = dynPreviewProducts[targetLabel];
  dynConfigureFirstSelectedTimingPreview(entry, mode);

  var layer = dynPreviewLayers[targetLabel];
  if (layer) {
    layer.setEeObject(entry.previewImage || entry.image);
    layer.setVisParams(entry.previewVis || entry.vis);
  }

  dynLegendMetricSpecs[targetLabel] = entry;

  if (
    dynamicsLegendMetricSelect !== null &&
    dynamicsLegendMetricSelect.getValue() === targetLabel
  ) {
    dynRenderLegend(targetLabel);
  }

  dynSetStatus(
    'Updated the first-observation timing preview to: ' + mode +
    '. The exported metric values are unchanged.'
  );
}
...
var dynamicsFirstSelectedTimingVis = ui.Select({
  items: [
  ...
  style: {stretch: 'horizontal'},
  onChange: function() {
    dynRefreshFirstSelectedTimingPreview();
  }
});

dynamicsVisualizationPanel.add(ui.Label({
  value: 'First-observation timing colour scheme',
  style: {fontWeight: 'bold', margin: '10px 0 2px 0'}
}));
dynamicsVisualizationPanel.add(dynamicsFirstSelectedTimingVis);
dynamicsVisualizationPanel.add(ui.Label({
  value: 'Recommended: two independent half-period ramps. For a 12-month hydrological year this gives one ramp to the first six months and a different ramp to the final six months. The multi-hue and original Viridis alternatives remain available for comparison.',
  style: {fontSize: '10px', color: '#555555', whiteSpace: 'normal'}
}));
...
function runAnnualDynamics() {
  ...
  if (dynFirstWet.getValue()) {
    var firstSelectedEntry = addSelected(
      firstSelected,
      timingViridisVis,
      'First observed selected-water day (analysis day)',
      'ANALYSIS_DAY'
    );
    firstSelectedEntry.isFirstSelectedTiming = true;
    dynConfigureFirstSelectedTimingPreview(
      firstSelectedEntry,
      dynamicsFirstSelectedTimingVis.getValue()
    );
  }
  ...
  lastDynamicsImage = ee.Image(stack)
    ...
    .set({
      ...
      first_selected_timing_visualization: dynamicsFirstSelectedTimingVis.getValue(),
